...
/** Watches ~/.claude/session-status/focus-request.json (written by the claude-sessions
 *  desktop app) and acts on the matching terminal tab: a left-click "focus" request jumps to
...
class FocusWatcher : ProjectActivity {
     ...
     *  single one — the most-recently-focused project's frame (or, failing that, the first
     *  open project) — so exactly one spawns. */
