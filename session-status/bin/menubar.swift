...
/// Custom session names (⌥-click to rename); shared with floatdash + record.py.
...
final class ClickTable: NSTableView {
    ...
    override func mouseDown(with event: NSEvent) {
        let r = row(at: convert(event.locationInWindow, from: nil))
        if r >= 0, event.modifierFlags.contains(.option) { onRename?(r); return }   // ⌥-click → rename
...
final class PopoverList: NSViewController, NSTableViewDataSource, NSTableViewDelegate {
    ...
    override func loadView() {
        ...
        tbl.onRename = { [weak self] r in self?.renameSession(r) }
        table = tbl
        table.style = .plain
    ...
    func renameSession(_ row: Int) {
        ...
        let s = sessions[row]
        onActivate?()   // close the popover so the dialog is front
        let alert = NSAlert()
